...
### Q: 需要什么环境？
A: 需要 Node.js 16+ 和 npm。如果要使用自动生成补丁功能，还需要 Java 11+。

### Q: 如何修改默认端口？
A: 编辑 `backend/.env` 文件，修改 `PORT=3000` 为你想要的端口。

### Q: 支持哪些数据库？
A: 默认使用 SQLite，也支持 MySQL。在 `.env` 文件中配置 `DB_TYPE`。

### Q: 如何部署到生产环境？
A: 推荐使用 Docker 部署，或者使用 PM2 管理 Node.js 进程。详见 README.md 的部署章节。
...
### Q: 上传补丁失败？
A: 检查文件大小是否超过限制（默认 100MB），检查 uploads 目录权限。

### Q: 生成补丁时提示"无需生成补丁"？
A: 这表示两个 APK 版本的内容完全相同，没有任何差异。请检查：
- 是否上传了相同的 APK 文件
- 新版本 APK 是否真的包含了代码或资源的修改
...
### Q: 客户端如何检查更新？
A: 调用 `/api/client/check-update` 接口，传入 appId、version 和 deviceId。

### Q: 如何获取 appId？
A: 在应用设置页面可以看到应用 ID，点击复制按钮即可。

### Q: 补丁下载失败怎么办？
A: 检查网络连接，检查补丁文件是否存在，检查 MD5 校验是否通过。

### Q: 如何上报下载结果？
A: 调用 `/api/client/report` 接口，传入 patchId、deviceId、success 等参数。

## 数据管理

### Q: 如何备份数据？
A: 在系统设置 -> 备份管理中，点击"立即备份"。也可以启用定时自动备份。

### Q: 如何恢复备份？
A: 在备份列表中，点击"恢复"按钮，确认后即可恢复。

### Q: 如何清理旧数据？
A: 在系统设置 -> 数据清理中，可以清理日志和下载记录。也可以启用定时自动清理。
...
### Q: 数据库性能优化？
A: 对于大量数据，建议使用 MySQL 替代 SQLite，并添加适当的索引。
